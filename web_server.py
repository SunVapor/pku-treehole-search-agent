...
def worker_thread(worker_id):
    """后台工作线程，处理队列中的任务"""
    print(f"[Web Server] 工作线程 {worker_id} 启动")
    ...
    while True:
        try:
            ...
            print(f"[Worker {worker_id}] 开始处理任务 {task_id}, 模式 {mode}, 用户 {user_id or '未登录'}")
...
def main():
    ...
    # 启动多个工作线程以支持并发请求
    NUM_WORKERS = 3  # 同时处理3个请求
    workers = []
    for i in range(NUM_WORKERS):
        worker = threading.Thread(target=worker_thread, args=(i+1,), daemon=True)
        worker.start()
        workers.append(worker)
    
    print(f"[Web Server] 已启动 {NUM_WORKERS} 个工作线程，支持并发处理\n")
    ...
    print(f"\n[Web Server] 服务器启动成功！")
    print(f"[Web Server] 本地访问: http://localhost:{port}")
    print(f"[Web Server] 内网访问: http://<your-server-ip>:{port}")
    print(f"\n按 Ctrl+C 停止服务器\n")
